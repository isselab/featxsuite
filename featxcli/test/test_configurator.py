...
class TestConfigurator:
    ...
    def test_traverseModel_binding_inconsistency(self, sample_configs_data, capsys):
        ...
        fx_model = {
            ...
            "sub": [
                {
                    "name": "FeatureA",
                    "bindingTimeAllowed": "Late",  # Different from config (Early)
                    "bindingModeAllowed": "Dynamic",  # Different from config (Static)
                    "excludes": ""
                }
            ]
        }
    ...
    def test_traverseModel_static_depends_on_dynamic(self, sample_configs_data, capsys):
        ...
        assert "Static and selected child ChildFeature depends on dynamic parent ParentFeature" in captured.out
    ...
    def test_traverseModel_early_depends_on_late(self, capsys):
        ...
        assert "Early child feature ChildFeature cannot depends on late parent ParentFeature" in captured.out
    ...
    def test_traverseModel_mutual_exclusion(self, capsys):
        ...
        assert "FeatureB and FeatureA exclude each other. They cannot be selected simultaneously" in captured.out
    ...
    def test_traverseModel_selected_child_deselected_parent(self, capsys):
        ...
        assert "Parent ParentFeature is deselected but child ChildFeature is selected" in captured.out

    @patch.object(Configurator, 'readFeatures')
    @patch.object(Configurator, 'readConfigs')
    @patch.object(Configurator, 'traverseModel')
    def test_checkRules_integration(self, mock_traverse, mock_read_configs, mock_read_features):
    ...
    def test_full_integration_valid_model(self, temp_model_dir, sample_features_data, sample_configs_data):
    ...
    def test_empty_excludes_handling(self):
        ...
        configs = {
            "configs": [
                {"name": "FeatureA", "isSelected": True, "bindingTime": "Early", "bindingMode": "Static"}
            ]
        }
